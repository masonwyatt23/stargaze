/**
 * POST /api/github/star — direct star endpoint.
 *
 * Used by flows outside of the swipe deck (e.g. the access-request modal
 * once a project becomes available, or the "Star this repo" button on a
 * project detail page). Requires an authed user with a stored GH token.
 *
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    return NextResponse.json({ starred: true }, { status: 200 });
  } catch (err) {
    if (err instanceof GitHubAuthError) {
      // Clear the dead token so the UI knows to prompt re-auth.
    ...
    console.error("[github/star] unexpected error", err);
